...
select e.employee_id, sum(quantity) from employees e join orders o on e.employee_id=o.employee_id
join order_details od on od.order_id=o.order_id
where e.employee_id = 3 and order_date >= '1998-01-01' and order_date <= '2023-11-25' group by e.employee_id
--65. 10 numaralı ID ye sahip ürünümden son 3 ayda ne kadarlık ciro sağladım?
select p.product_name, sum(p.unit_price * od.quantity) from products p join order_details od
on od.product_id = p.product_id
join orders o on o.order_id=od.order_id
where p.product_id = 3
and order_date>= '1998-03-01' and order_date <= '1998-06-30' group by p.product_name
--66. Hangi çalışan şimdiye kadar toplam kaç sipariş almış..?
select e.first_name || ' ' || e.last_name as persons, count(o.order_id) from employees e
join orders o on o.employee_id=e.employee_id
join order_details od on od.order_id=o.order_id
